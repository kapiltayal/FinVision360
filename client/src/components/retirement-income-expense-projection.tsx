  ...
  return (
    <div className="group flex items-start gap-3 border-b border-current/10 px-4 py-3 last:border-b-0">
      ...
      <div className="flex shrink-0 items-center gap-1">
        <span className={`min-w-[6.5rem] text-right text-sm font-bold tabular-nums ${style.color}`}>{money(item.monthlyAmount)}</span>
        {item.isProjectionOnly ? (
          <span className="ml-1 flex w-16 justify-end opacity-100 transition-opacity sm:opacity-0 sm:group-hover:opacity-100">
            <Button size="icon" variant="ghost" className="h-7 w-7" onClick={onEdit} aria-label={`Edit ${item.name}`}><Pencil className="h-3.5 w-3.5" /></Button>
            <Button size="icon" variant="ghost" className="h-7 w-7 text-destructive" onClick={onDelete} aria-label={`Delete ${item.name}`}><Trash2 className="h-3.5 w-3.5" /></Button>
          </span>
        ) : <span aria-hidden="true" className="ml-1 w-16" />}
      </div>
    </div>
  );
}
...
function DebtPaymentsSection({ items }: { items: CashflowItem[] }) {
  const [open, setOpen] = useState(false);
  const total = items.reduce((sum, item) => sum + item.monthlyAmount, 0);
  return <div className="border-b border-rose-200/60 bg-rose-50/30 dark:border-rose-900/60 dark:bg-rose-950/10">
    <button type="button" className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left hover:bg-rose-50/70 dark:hover:bg-rose-950/20" onClick={() => setOpen(!open)} aria-expanded={open}>
      <span className="flex items-center gap-2">
        <ChevronDown className={`h-4 w-4 transition-transform ${open ? "rotate-180" : ""}`} />
        <span>
          <span className="block text-sm font-semibold">Debt payments</span>
          <span className="block text-xs text-muted-foreground">{items.length} payment{items.length === 1 ? "" : "s"}</span>
        </span>
      </span>
      <span className="text-sm font-bold tabular-nums text-rose-700 dark:text-rose-300">{money(total)}</span>
    </button>
    {open && <div className="border-t border-rose-200/60 dark:border-rose-900/60">
      {items.map((item) => <CashflowRow key={item.id} item={item} onEdit={() => undefined} onDelete={() => undefined} />)}
    </div>}
  </div>;
}
...
function Column({ kind, items, onAdd, onEdit, onDelete, expected, onExpectedChange, onExpectedBlur, savingExpected, onWithdrawalRateChange, savingWithdrawalAssetId }: {
  ...
  const debtPayments = items.filter((item) => item.source === "debt-payment");
  const displayItems = items.filter((item) => item.source !== "expected-expenses" && item.source !== "retirement-account" && item.source !== "pension" && item.source !== "debt-payment");
  return (
    <section className={`overflow-hidden rounded-2xl border ${income ? "border-teal-200/80 dark:border-teal-900" : "border-amber-200/80 dark:border-amber-900"}`}>
      ...
      <div className="bg-background">
        ...
        {!income && debtPayments.length > 0 && <DebtPaymentsSection items={debtPayments} />}
